    ...
    <Dialog as="div" @close="closeModal" class="relative z-modal">
      ...
      <div class="fixed inset-0 overflow-y-auto">
        <div class="flex min-h-full items-center justify-center p-4">
          ...
          >
            <DialogPanel class="transform overflow-visible rounded-lg bg-color-bg-neutral shadow-2xl transition-all preview-image-modal" style="max-width: 90vw; max-height: 90vh;">
              <!-- Header -->
              ...
              <div class="p-0">
                ...
                <div v-if="isLoading" class="flex items-center justify-center py-16">
                  <div class="text-center">
                    ...
                    <p class="text-sm text-color-text-secondary">Loading image...</p>
                  </div>
                </div>
                ...
                <div v-else-if="error" class="flex items-center justify-center py-16">
                  <div class="text-center">
                    ...
                    <h2 class="text-xl font-semibold text-color-text-primary mb-2">Failed to load image</h2>
...
async function loadImage() {
  ...
  try {
    ...
    const { attachmentFilename, noteData } = await loadNoteDataAndAttachment(basename);
    
    // Debug: Log the values to see what's happening
    console.log('Note data:', noteData);
    console.log('Attachment extension from note:', noteData.attachment_extension);
    console.log('Category from note:', noteData.category);
    console.log('Generated attachment filename:', attachmentFilename);
    
    // If attachment_extension is not available, try to extract from title
    if (!noteData.attachment_extension && noteData.title) {
      const titleExtension = noteData.title.split('.').pop().toLowerCase();
      if (['jpg', 'jpeg', 'png', 'gif', 'bmp', 'webp'].includes(titleExtension)) {
        // Use the extension from title
        const correctFilename = `${basename}/image.${titleExtension}`;
        console.log('Using extension from title:', titleExtension);
        console.log('Corrected filename:', correctFilename);
        imageUrl.value = `/files/${correctFilename}`;
        return;
      }
    }
    
    // Construct image URL
    imageUrl.value = `/files/${attachmentFilename}`;
...
function onImageError() {
  error.value = 'Failed to load image. The file may be corrupted or not accessible.';
  isLoading.value = false;
}
